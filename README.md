...
本项目提供三种使用方式：
1. **[推荐] Web网页版**: 直接双击 `index.html` 在浏览器中使用，无需安装任何环境。
2. **Mac一键版**: 双击 `一键批量处理.command` 运行。
3. **命令行版**: 适合开发者或需要集成到脚本中。

### 方式一：Web网页版 (index.html)
- **在线使用**: [点击这里直接使用](https://bullshitai52.github.io/smart-id-photo-cropper/) (推荐)
- 或者双击打开本地的 `index.html`。
- 将图片拖入网页。
- 设置尺寸后点击下载（支持打包下载）。
- **特点**: 纯本地运行，图片不会上传服务器，保护隐私。
